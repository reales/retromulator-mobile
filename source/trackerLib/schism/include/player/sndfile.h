...
/* Trackermeister: fractional tick length with a host tempo scale */
static inline uint32_t csf_calculate_tick_length(song_t *csf)
{
	double scale = (csf->tm_tempo_scale > 0.0) ? csf->tm_tempo_scale : 1.0;
	double len = ((double)csf->mix_frequency * 2.5)
		/ ((double)csf->current_tempo * scale) + csf->tm_tick_frac;
	uint32_t n = (uint32_t)len;
	csf->tm_tick_frac = len - (double)n;
	return n ? n : 1;
}
