...
// Trackermeister: MOD channels take the Amiga L-R-R-L hard panning
bool tmAmigaPan = false;

static uint8_t tmAmigaChannelPan(const channel_t *ch)
{
	const int32_t i = (int32_t)(ch - channel) & 3;
	return (i == 0 || i == 3) ? 0 : 255;
}
song_t song;
instr_t *instr[128+4];
...
void resetChannels(void)
{
	...
	for (int32_t i = 0; i < MAX_CHANNELS; i++, ch++)
	{
		ch->instrPtr = instr[0];
		ch->status = CS_UPDATE_VOL;
		ch->oldPan = ch->outPan = ch->finalPan = tmAmigaPan ? tmAmigaChannelPan(ch) : 128;
...
void triggerNote(uint8_t note, uint8_t efx, uint8_t efxData, channel_t *ch)
{
	...
	ch->oldVol = s->volume;
	ch->oldPan = tmAmigaPan ? tmAmigaChannelPan(ch) : s->panning;
	...
	if (song.row >= song.currNumRows || song.posJumpFlag)
	{
		...
		if (playMode != PLAYMODE_PATT && playMode != PLAYMODE_RECPATT)
		{
			...
			else if (++song.songPos >= song.songLength)
			{
				...
				tmSongWrapFlag = true; // Trackermeister
