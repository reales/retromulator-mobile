...
void setMixerBPM(int32_t bpm)
{
	...
	// Trackermeister: host tempo scale, clamped to the mix buffer size
	if (tmTempoScale != 1.0 && tmTempoScale > 0.0)
	{
		double dLen = ((double)audio.samplesPerTickInt + ((double)audio.samplesPerTickFrac / (double)BPM_FRAC_SCALE)) / tmTempoScale;
		if (dLen < 1.0) dLen = 1.0;
		if (tmMaxTickSamples > 0 && dLen > (double)tmMaxTickSamples) dLen = (double)tmMaxTickSamples;
		audio.samplesPerTickInt = (uint32_t)dLen;
		audio.samplesPerTickFrac = (uint64_t)((dLen - (double)audio.samplesPerTickInt) * (double)BPM_FRAC_SCALE) & BPM_FRAC_MASK;
	}
...
static void voiceTrigger(int32_t ch, sample_t *s, int32_t position)
{
	...
	v->mixFuncOffset = ((int32_t)sample16Bit * (NUM_INTERPOLATORS*3)) + (audio.interpolationType * 3) + loopType;
	v->active = true;
}
...
// ---------------------------------------------------------------------------
// Trackermeister: headless block render, the same loop as audioCallback()
// ---------------------------------------------------------------------------
...
bool tmSetupAudioBuffers(uint32_t maxFreq)
{
	tmMaxTickSamples = (uint32_t)ceil(maxFreq / (MIN_BPM / 2.5)) + 1;
	audio.fMixBufferL = (float *)calloc(tmMaxTickSamples, sizeof (float));
	audio.fMixBufferR = (float *)calloc(tmMaxTickSamples, sizeof (float));
	tmVoiceBufL = (float *)calloc(tmMaxTickSamples, sizeof (float));
	tmVoiceBufR = (float *)calloc(tmMaxTickSamples, sizeof (float));
	return audio.fMixBufferL != NULL && audio.fMixBufferR != NULL;
}

void tmSetTempoScale(double scale)
{
	tmTempoScale = scale;
	setMixerBPM(song.BPM);
}

static void tmMarkVisited(void)
{
	const uint32_t key = ((uint32_t)(song.songPos & 0xFF) << 8) | (uint32_t)(song.row & 0xFF);
	tmVisited[key >> 3] |= (uint8_t)(1 << (key & 7));
}

void tmResetSongEnd(bool stopAtEnd)
{
...
bool tmSongHasEnded(void)
{
	return tmSongEnded;
}

// frames must not exceed the mix buffer (the caller renders in small blocks)
void tmRenderFloat(float *stream, uint32_t frames)
{
	if (frames > tmMaxTickSamples)
		frames = tmMaxTickSamples;

	int32_t bufferPosition = 0;
	uint32_t samplesLeft = frames;
	while (samplesLeft > 0)
	{
		if (audio.tickSampleCounter == 0)
		{
			...
			if (tmOrderEntryFlag)
			{
				...
				const uint32_t key = ((uint32_t)(song.songPos & 0xFF) << 8) | (uint32_t)(song.row & 0xFF);
				const bool wrapped = tmSongWrapFlag;
				tmSongWrapFlag = false;
				if (wrapped || (tmVisited[key >> 3] & (1 << (key & 7))))
				{
					if (tmStopAtEnd)
						tmEndPending = true;
					else
	...
	sendSamples32BitFloatStereo(stream, frames);
}

// 0..1 audio peak of a channel since the last read, for the meter
float tmGetVoiceLevel(int32_t ch)
{
	if (ch < 0 || ch >= song.numChannels)
		return 0.0f;
	float f = tmVoicePeak[ch];
	tmVoicePeak[ch] = 0.0f;
