...
void *resample_dup(const void *	handle)
{
   ...
   hp->Yp = cpy->Yp;
   hp->Time = cpy->Time;
   hp->interpFilt = cpy->interpFilt;

   return (void *)hp;
}

static void *resample_open_internal(int Nmult, double minFactor, double maxFactor)
{
   double *Imp64;
   double Rolloff, Beta;
   rsdata *hp;
   ...
   hp->interpFilt = TRUE; /* default: interpolate filter coefficients */
...
{
   return resample_open_internal(highQuality ? 35 : 11, minFactor, maxFactor);
}

void *resample_open_custom(int filterTaps, double minFactor, double maxFactor)
{
   return resample_open_internal(filterTaps, minFactor, maxFactor);
}

void resample_set_interp(void *handle, int interp)
{
   rsdata *hp = (rsdata *)handle;
   hp->interpFilt = interp ? TRUE : FALSE;
}
                     ...
                     int     outBufferLen)
{
   ...
   *inBufferUsed = 0;
   ...
   if (hp->Yp && (outBufferLen-outSampleCount)>0) {
      ...
      memcpy(&outBuffer[outSampleCount], hp->Y, len * sizeof(float));
      outSampleCount += len;
      hp->Yp -= len;
      if (hp->Yp > 0)
         memmove(hp->Y, &hp->Y[len], hp->Yp * sizeof(float));
   }
   ...
   for(;;) {
      ...
      memcpy(&hp->X[hp->Xread], &inBuffer[*inBufferUsed], len * sizeof(float));
      ...
      *inBufferUsed += len;
      ...
      memmove(hp->X, &hp->X[hp->Xp - hp->Xoff], Nreuse * sizeof(float));
