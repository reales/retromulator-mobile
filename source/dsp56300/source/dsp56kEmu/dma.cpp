...
//				m_pendingTransfer = 1;
		...
		else
		{
			...
			[[maybe_unused]] const auto prio = getPriority();
			const auto reqSrc = getRequestSource();
			[[maybe_unused]] const auto addrModeSrc = getSourceAddressGenMode();
			[[maybe_unused]] const auto addrModeDst = getDestinationAddressGenMode();
			[[maybe_unused]] const auto srcSpace = getSourceSpace();
			[[maybe_unused]] const auto dstSpace = getDestinationSpace();
...
//			auto& mem = dsp.memory();
	...
	bool DmaChannel::execTransfer()
	{
		...
		if(bitvalue(m_dcr, D3d))
		{
			...
			{
				[[maybe_unused]] const auto prev = _target;
