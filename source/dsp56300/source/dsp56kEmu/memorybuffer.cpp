...
namespace dsp56k
{
	std::string MemoryBuffer::g_tempPath;
...
//		const auto maxAreaByteSize = maxAreaSize * static_cast<TWord>(sizeof(TWord));
	...
	MemoryBuffer::THandle MemoryBuffer::createFileMapping(const TWord _neededByteSize)
	{
		...
		{
			// shm_open is not available in sandboxed environments (e.g. iOS).
			// Fall back to a temp file in the container directory.
			LOG("shm_open failed (err " << errno << "), falling back to tmp file");

			if(g_tempPath.empty())
			{
				LOG("No temp path set, cannot create file-backed mapping");
				return InvalidHandle;
			}

			std::string tmpPath = g_tempPath + "/dsp56300_XXXXXX";
			fd = mkstemp(&tmpPath[0]);
			if(fd == InvalidHandle)
			{
				LOG("mkstemp failed in " << g_tempPath << ", err " << errno);
				return InvalidHandle;
			}
