...
static bool hasHardenedRuntime() noexcept {
  ...
  // iOS always requires MAP_JIT — W^X is enforced system-wide.
  return true;
#else
...
#elif defined(__APPLE__) && TARGET_OS_IOS
  // iOS supports MAP_JIT when the process has the dynamic-codesigning entitlement.
  return true;
#else
  // MAP_JIT is not available (it's only available on OSX/iOS).
  return false;
#endif
}
