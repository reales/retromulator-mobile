	...
	std::string ParameterDescriptions::parseValueList(const std::string& _key, const juce::var& _values)
	{
		...
		else
		{
			...
			for (const auto& it : valueMap->getProperties())
			{
				...
				const auto keyLong = std::strtol(keyName.c_str(), nullptr, 10);

				if(keyLong < 0)
				{
					return std::string("Invalid parameter index '") + keyName + " in value list '" + _key + "', values must be >= 0";
				}
				const auto key = static_cast<int>(keyLong);
				if(knownValues.find(static_cast<unsigned int>(key)) != knownValues.end())
				{
					return std::string("Parameter index '") + keyName + " in value list '" + _key + " has been specified twice";
				}
				knownValues.insert(static_cast<unsigned int>(key));

				const auto requiredLength = key+1;

				if(static_cast<int>(vl.texts.size()) < requiredLength)
					vl.texts.resize(static_cast<size_t>(requiredLength));

				vl.texts[static_cast<size_t>(key)] = value.toString().toStdString();
	...
	void ParameterDescriptions::parseMidiPacket(std::stringstream& _errors, const std::string& _key, const juce::var& _value)
	{
		...
		for(auto i=0; i<arr->size(); ++i)
		{
			...
			else if(type == "param")
			{
				...
				if(hasMask)
				{
					const int mask = static_cast<int>(strtol(entry["mask"].toString().toStdString().c_str(), nullptr, 16));
					if(mask < 0 || mask > 0xff)
					{
