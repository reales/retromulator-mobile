...
namespace n2x
{
	...
	, m_boot(m_dsp)
	{
...

		{
			...
			// iOS runs in interpreter mode which does not advance m_cycles,
			// only m_instructions.  Use Instructions as the clock source
			// so the ESAI clock actually fires.
			// Increase speed percent by 50% to reduce the output sample rate
			// (~98200 -> ~65467 Hz), giving the firmware more instructions
			// per sample for poly patches while staying within throughput.
#if TARGET_OS_IPHONE
			clock.setClockSource(dsp56k::EsaiClock::ClockSource::Instructions);
			clock.setSpeedPercent(200);
#else
