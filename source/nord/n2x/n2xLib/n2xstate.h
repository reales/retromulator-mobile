...
namespace n2x
{
	...
	public:
		...
		static uint32_t getNameLengthInDump(const synthLib::SysexBuffer& _dump);
		...
		// Embed a patch name into a sysex buffer using the Aura editor extension format.
		// The name is placed as 20 ASCII bytes immediately before the F7 terminator.
		// If the dump already has a name (10 or 20 bytes) it is replaced.
		// The name is space-padded or truncated to exactly g_nameLength (20) characters.
		static synthLib::SysexBuffer writePatchName(const synthLib::SysexBuffer& _dump, const std::string& _name);
