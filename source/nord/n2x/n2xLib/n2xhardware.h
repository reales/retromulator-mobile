...
namespace n2x
{
	...
	public:
		...
		uint32_t getMidiOffsetCounter() const { return m_midiOffsetCounter; }
		size_t getOutputBufferSize();
		void notifyBootFinished();

		const std::string& getRomFilename() const { return m_rom.getFilename(); }

		void setEffectiveSamplerate(double _samplerate) { m_samplerateInv = 1.0 / _samplerate; }
	...
	private:
		...
		// Debug counters for sync diagnostics
		uint32_t m_debugCallbackCount = 0;
		uint32_t m_debugUnderrunCount = 0;
	};
}
