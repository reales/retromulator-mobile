...
namespace n2x
{
	...
	{
		return _dump.size() == g_singleDumpSize
			|| _dump.size() == g_singleDumpWithLegacyNameSize
			|| _dump.size() == g_singleDumpWithNameSize;
	}

	bool State::isMultiDump(const synthLib::SysexBuffer& _dump)
	{
		return _dump.size() == g_multiDumpSize
			|| _dump.size() == g_multiDumpWithLegacyNameSize
			|| _dump.size() == g_multiDumpWithNameSize;
	}

	uint32_t State::getNameLengthInDump(const synthLib::SysexBuffer& _dump)
	{
		if (_dump.size() == g_singleDumpWithNameSize || _dump.size() == g_multiDumpWithNameSize)
			return g_nameLength;
		if (_dump.size() == g_singleDumpWithLegacyNameSize || _dump.size() == g_multiDumpWithLegacyNameSize)
			return g_legacyNameLength;
		return 0;
	}

	std::string State::extractPatchName(const synthLib::SysexBuffer& _dump)
	{
		const auto nameLen = getNameLengthInDump(_dump);
		if (nameLen == 0)
			return {};
		auto* begin = &_dump[_dump.size() - nameLen - 1];
		if (*begin == 0xf7)
			return {};
		std::string name(reinterpret_cast<const char*>(begin), nameLen);
	...
	{
		return getNameLengthInDump(_dump) > 0;
	}
	...
	synthLib::SysexBuffer State::stripPatchName(const synthLib::SysexBuffer& _dump)
	{
		...
		if (nameLen == 0)
			return _dump;
		auto d = _dump;
		d.erase(d.end() - static_cast<int>(nameLen) - 1, d.end() - 1);
	...
	bool State::isValidPatchName(const synthLib::SysexBuffer& _dump)
	{
		...
		const auto nameStart = _dump.size() - nameLen - 1;

		for (size_t i = nameStart; i < nameStart + nameLen; ++i)
		{
	...
	synthLib::SysexBuffer State::writePatchName(const synthLib::SysexBuffer& _dump, const std::string& _name)
	{
		// Start from a strip so we always work with the standard-size dump
		auto d = stripPatchName(_dump);

		// Build the 20-byte name: space-padded, ASCII printable clamp
		std::string name(_name);
		name.resize(g_nameLength, ' ');
		for(auto& ch : name)
		{
			if(static_cast<unsigned char>(ch) < 32 || static_cast<unsigned char>(ch) >= 128)
				ch = ' ';
		}

		// Remove the F7 terminator, append 20 name bytes, restore F7
		d.back() = static_cast<uint8_t>(name[0]);
		for(size_t i = 1; i < g_nameLength; ++i)
			d.push_back(static_cast<uint8_t>(name[i]));
		d.push_back(0xf7);
