...
//		m_dsp->getMemory().saveAssembly("P.asm", 0, m_dsp->getMemory().sizeP(), true, false, m_dsp->getDSP().getPeriph(0), m_dsp->getDSP().getPeriph(1));
		...
		default:
			{
				fprintf(stderr, "[Virus] Device: waiting for DSP boot (model %d)…\n", static_cast<int>(m_rom.getModel()));
				uint32_t bootLoops = 0;
				while(!m_mc->dspHasBooted())
				{
					dummyProcess(8);
					if(++bootLoops % 1000 == 0)
						fprintf(stderr, "[Virus] Device: boot loop iteration %u…\n", bootLoops);
				}
				fprintf(stderr, "[Virus] Device: boot finished after %u iterations\n", bootLoops);
			}
		}

		fprintf(stderr, "[Virus] Device: sending init commands…\n");
		m_mc->sendInitControlCommands(127);

		dummyProcess(8);

		fprintf(stderr, "[Virus] Device: creating default state…\n");
		m_mc->createDefaultState();
		fprintf(stderr, "[Virus] Device: construction complete\n");
	}
...
//			LOG("MIDI: " << std::hex << (int)_ev.a << " " << (int)_ev.b << " " << (int)_ev.c);
	...
	void Device::configureDSP(DspSingle& _dsp, const ROMFile& _rom, const float _samplerate)
	{
		...
		if(_rom.isTIFamily())
		{
			...
			// iOS interpreter mode does not advance m_cycles, use Instructions
#if TARGET_OS_IPHONE
			clock.setClockSource(dsp56k::EsaiClock::ClockSource::Instructions);
#else
