...
[[maybe_unused]] constexpr uint8_t g_pageC_global[] = {45,  63,  64,  65,  66,  67,  68,  69,  70,  85,  86,  87,  90,  91,
...
bool Microcontroller::sendSysex(const synthLib::SysexBuffer& _data, std::vector<SMidiEvent>& _responses, const MidiEventSource _source)
{
	...
	auto buildSingleBankResponse = [&](const BankNumber _bank)
	{
		...
		if(bankIndex < m_singles.size())
		{
			...
			{
				TPreset data;
				[[maybe_unused]] const auto res = requestSingle(_bank, i, data);
	...
	auto buildMultiBankResponse = [&](const BankNumber _bank)
	{
		if(_bank == BankNumber::A)
		{
			...
			{
				TPreset data;
				[[maybe_unused]] const auto res = requestMulti(_bank, i, data);
