...
namespace synthLib
{
	...
	void ResamplerInOut::process(const TAudioInputs& _inputs, TAudioOutputs& _outputs, const TMidiVec& _midiIn, TMidiVec& _midiOut, const uint32_t _numSamples, const TProcessFunc& _processFunc)
	{
		...
		if (outputSize != _numSamples)
		{
			fprintf(stderr, "[ResamplerInOut] OUTPUT MISMATCH: produced=%u, requested=%u, devSR=%.0f, hostSR=%.0f\n",
				outputSize, _numSamples, m_samplerateDevice, m_samplerateHost);
		}
