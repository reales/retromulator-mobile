...
namespace synthLib
{
	...
	protected:
		...
		// Override to limit polyphony. Default 128 = no limit.
		virtual uint8_t getMaxPolyphony() const { return 128; }

		static constexpr uint8_t kDefaultMaxPolyphony = 4;
	...
	private:
		...
		// Polyphony limiter state
		std::array<std::array<bool, 128>, 16> m_noteMatrix{};
		std::array<std::deque<uint8_t>, 16> m_noteOrder;
		std::vector<SMidiEvent> m_stolenNoteOffs;
	};
}
