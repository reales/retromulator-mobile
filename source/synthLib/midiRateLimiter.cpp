...
namespace synthLib
{
	...
	void MidiRateLimiter::processSample()
	{
		...
		// Flush all pending realtime (note-on/off, CC, PC) immediately, even mid-sysex.
		// Realtime events must not be held hostage behind a slow sysex transmission.
		if (!m_pendingRealtime.empty() && !m_sendingSysex)
		{
			while (!m_pendingRealtime.empty())
			{
				...
				if (len > 0) m_writeCallback(e.a);
				if (len > 1) m_writeCallback(e.b);
				if (len > 2) m_writeCallback(e.c);
			}
		}

		if (m_pendingBytes.empty())
		{
			if (!m_pendingSysex.empty())
			{
