...
namespace sc88smf
{
	...
	// Parses an in-memory SMF into _events, sorted by time. _name only labels errors.
	// Returns false and fills _error if the data is not a readable SMF.
	inline bool parse(const std::vector<uint8_t>& file, const std::string& _path,
	                  std::vector<Event>& _events, std::string& _error)
	{
		_events.clear();
	...
	inline bool read(const std::string& _path, std::vector<Event>& _events, std::string& _error)
	{
		...
		return parse(file, _path, _events, _error);
	}
}
