...
namespace emu88Lib
{
	...
	void Sc88Thread::processJob(ProcessJob& _job)
	{
		...
		uint32_t rendered = 0;
		for(uint32_t i = 0; i < _job.samplesToProcess && !m_exit.load(std::memory_order_acquire); ++i, ++rendered)
		{
		...
		m_renderedSamples.fetch_add(rendered, std::memory_order_relaxed);
		m_renderNanoseconds.fetch_add(static_cast<uint64_t>(
			std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::steady_clock::now() - renderStart).count()),
