...
namespace emu88Lib
{
	...
	// Render
	// =====================================================================
	Sc88::SampleFrame Sc88::renderXpAudioFrame()
	{
		const auto& dsp = m_xp.dsp();
		const auto& bus = dsp.serialOutput(xpLib::Dsp::SerialBus::c);
		if(dsp.serialOutputCount(xpLib::Dsp::SerialBus::c) >= 2)
		{
			m_lastXpFrame = {xpLib::XP::serialWordToOutput(bus[0]), xpLib::XP::serialWordToOutput(bus[1])};
			m_heldXpFrames = 0;
			return m_lastXpFrame;
		}
		// No pair this step: repeat the previous word as the DAC would, but not forever,
		// so a stopped DSP program does not leave a DC level on the output.
		++m_missingXpFrames;
		return ++m_heldXpFrames <= 64 ? m_lastXpFrame : SampleFrame{0, 0};
	}
