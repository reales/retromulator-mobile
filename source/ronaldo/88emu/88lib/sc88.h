...
namespace emu88Lib
{
	...
	public:
		...
		// Steps on which the XP emitted no stereo pair while enabled, since the last call.
		// The DAC repeats its last word on such a step; a run of zeros would be a click.
		uint32_t takeMissingXpFrames() { const auto n = m_missingXpFrames; m_missingXpFrames = 0; return n; }
	...
	protected:
		SampleFrame renderXpAudioFrame();
		...
		SampleFrame m_lastXpFrame{};
		uint32_t m_heldXpFrames = 0;
		uint32_t m_missingXpFrames = 0;
