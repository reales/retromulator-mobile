...
namespace emu88Lib
{
	...
	public:
		...
		// Offline render: processAudio waits for the worker instead of covering an
		// empty ring with the last frame. Only while no real-time consumer is running.
		void setOfflineRender(const bool _offline) { m_offlineRender.store(_offline, std::memory_order_relaxed); }
	...
	private:
		...
		// Held across an underrun: dropping to zero for a few samples is a click, while
		// repeating the last frame is inaudible at these lengths.
		std::pair<int32_t, int32_t> m_lastFrame{};
		uint32_t m_underrunSamples = 0;
		std::atomic<bool> m_offlineRender{false};
		uint32_t m_underrunWindow = 0;
