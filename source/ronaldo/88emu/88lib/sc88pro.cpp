...
namespace emu88Lib
{
	...
	Sc88Pro::SampleFrame Sc88Pro::renderSample()
	{
		...
		SampleFrame newOutput{0, 0};
		if(xpDsp.serialOutputCount(xpLib::Dsp::SerialBus::c) >= 2)
		{
			newOutput = {xpLib::XP::serialWordToOutput(busC[0]), xpLib::XP::serialWordToOutput(busC[1])};
			m_lastXpFrame = newOutput;
			m_heldXpFrames = 0;
		}
		else
		{
			// No pair this step: repeat the previous word as the DAC would, bounded so a
			// stopped DSP program does not leave a DC level on the output.
			++m_missingXpFrames;
			if(++m_heldXpFrames <= 64)
				newOutput = m_lastXpFrame;
		}
