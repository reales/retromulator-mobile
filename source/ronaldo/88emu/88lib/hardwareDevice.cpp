...
namespace emu88Lib
{
	...
	namespace
	{
		// Universal Non-Realtime MIDI Tuning Standard bulk dump: f0 7e <dev> 08 01 ...
		// No SC board acts on one, and each is 408 bytes. Logic repeats it thousands of
		// times a second on its AU path, far more than the SC-8850's USB port can drain,
		// so the sub-MCU queue grows without bound and real notes never reach the voices.
		// The boards that take MIDI on a serial port absorb it harmlessly, which is why
		// only the 8850 fell silent.
		bool isTuningBulkDump(const std::vector<uint8_t>& _sysex)
		{
			return _sysex.size() >= 5 && _sysex[0] == 0xf0 && _sysex[1] == 0x7e
			    && _sysex[3] == 0x08 && _sysex[4] == 0x01;
		}
	}
	...
	{
		if(isTuningBulkDump(_event.sysex))
			return true;
	...
	std::pair<int32_t, int32_t> HardwareDevice::renderBoardSample()
	{
		...
		++m_renderedSamples;
		// Nothing in this app reads the panel faster than a few times a second.
		if((m_renderedSamples & 1023u) == 0)
	                                  ...
	                                  const size_t _samples)
	{
		m_thread->processSamples(static_cast<uint32_t>(_samples), getExtraLatencySamples(),
		                         m_midiIn, m_midiOut);
		const bool offline = m_offlineRender.load(std::memory_order_relaxed);
		for(size_t i = 0; i < _samples; ++i)
		{
			Sc88Thread::SampleFrame frame{};
			bool got = m_thread->popSample(frame);
			if(!got && offline)
			{
				// Nothing is listening in real time, so the correct answer to an empty
				// ring is to wait for the worker rather than repeat the last frame. The
				// deadline only guards against a worker that has stopped.
				const auto deadline = std::chrono::steady_clock::now() + std::chrono::seconds(5);
				while(!got && std::chrono::steady_clock::now() < deadline)
				{
					std::this_thread::sleep_for(std::chrono::microseconds(200));
					got = m_thread->popSample(frame);
				}
			}
			if(got)
				m_lastFrame = frame;
		...
		// An empty output ring is what a dropout sounds like; report it so a crackle can be
		// told apart from a resampling fault.
		m_underrunWindow += static_cast<uint32_t>(_samples);
		if(m_underrunWindow >= 5 * 32000)
		{
			uint64_t rendered = 0, nanos = 0;
			m_thread->takeRenderStats(rendered, nanos);
			const double load = rendered ? (static_cast<double>(nanos) * 1e-9) / (static_cast<double>(rendered) / getSamplerate()) * 100.0 : 0.0;
			const auto missing = m_sc88 ? m_sc88->takeMissingXpFrames()
			                   : m_sc88Pro ? m_sc88Pro->takeMissingXpFrames()
			                   : m_sc8850 ? m_sc8850->takeMissingXpFrames() : 0u;
			const auto discarded = m_thread->takeDiscardedFrames();
			std::fprintf(stderr, "[88emu] audio: render load %.0f%%, %u underrun, %u missing XP frames, %llu discarded, of %u samples\n",
				load, m_underrunSamples, missing, static_cast<unsigned long long>(discarded), m_underrunWindow);
			m_underrunSamples = 0;
			m_underrunWindow = 0;
		}
	}
