...
{
public:
  ESPOptimizer(ESP<lg2eram_size>* esp) : m_esp(esp), logger(nullptr)
  {
...
//	code.setLogger(&logger);
    ...
    {
      if (withEram)
        eramEmitter.emit(static_cast<int>(pc), jit, m_asm);
      emitter->emit(static_cast<int>(pc), jit, m_asm);
    }
    ...
    bool lastMul30 = false;
		...
		void pre_optimize()
		{
			...
			for (int pc = 0; pc < PRAM_SIZE; pc++) // flatten accA and accB into single array
			{
				...
				// readReg is resolved here, after destReg offsets are finalised, so
				// the value already includes the +3 for accB — no separate adjustment needed.
				if (a.save)
				{
					int rd = pram_opt[a.writePC].m_access.destReg;
					// If the source instruction was never assigned a register (nop or
					// writePC search failed), fall back to the current acc (srcReg).
					a.readReg = (rd >= 0) ? rd : a.srcReg;
					assert(a.readReg >= 0 && a.readReg < 6);
				}
			}
