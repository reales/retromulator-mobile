...
namespace jeLib
{
	...
	, m_midiInRateLimiter([this](const uint8_t _byte) { midi.provideMIDI(&_byte, 1); })
	{
		...
		m_midiInRateLimiter.setSamplerate(44100.0f);
	...
	void Je8086::step()
	{
		...
		asics.runForCycles(emu.getCycles() * 1323 / 1250); // Convert from uC cycles to DSP steps at 44100 Hz (half of 88200 Hz rate)
	}
				...
				switch (ctr2++)
				{
					...
					default:
						...
						if (ctr2 > 60)
						{
							...
							{
								const auto sep = _ramDataFilename.find_last_of("/\\");
								if (sep != std::string::npos)
									baseLib::filesystem::createDirectory(_ramDataFilename.substr(0, sep));
							}
