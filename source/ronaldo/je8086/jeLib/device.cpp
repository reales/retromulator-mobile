...
namespace jeLib
{
	...
	{
		const auto ramDataFilename = _params.homePath.empty() ? "ram_dump.bin" : _params.homePath + "/ROM/ram_dump.bin";
	...
	bool Device::setState(const std::vector<uint8_t>& _state, synthLib::StateType _type)
	{
		...
		// if master volume was not part of the state, default to 12.0f (matches constructor default / intended output level)
		if (m_masterVolume < 0)
			m_masterVolume = 12.0f;
