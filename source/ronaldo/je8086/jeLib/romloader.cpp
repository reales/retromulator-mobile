...
namespace jeLib
{
	...
	Rom RomLoader::loadFromMidiFiles(const std::vector<MidiData>& _files)
	{
		...
		// Determine output filename based on device type.
		const std::string binName = (fullRom.size() == Rom::RomSizeRack) ? "jp8000rack.bin" : "jp8000.bin";

		// Write the assembled ROM next to the source files and delete the .mid dumps.
		if (!_files.empty() && !_files.front().first.empty())
		{
			const auto dir = validatePath(getPath(_files.front().first));
			const auto outPath = dir + binName;
			if (writeFile(outPath, fullRom))
			{
				for (const auto& [path, data] : _files)
					std::remove(path.c_str());
			}
		}

		return Rom(fullRom, binName);
	}
