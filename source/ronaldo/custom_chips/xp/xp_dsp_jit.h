...
// No backend is compiled in; see the dispatcher stub below.
...
// Platforms that forbid generating code at run time (iOS) run the naive engine only. acquire() returning
// null is the path Dsp::runJitFrame already takes while a compile is pending, so the frame driver needs no
// change; there is simply never any compiled code to adopt. No backend, no worker thread, no asmjit.
namespace xpLib
{
	class DspJitDispatcher
	{
	public:
		DspJitDispatcher() = default;

		DspJitDispatcher(const DspJitDispatcher&) = delete;
		DspJitDispatcher& operator=(const DspJitDispatcher&) = delete;

		DspJitRun acquire(uint64_t, const FlatProgram&, const FlatProgram*) { return nullptr; }
	};
} // namespace xpLib
